...
def process(tlist):
    ...
    try:
        ib.connect('0.0.0.0', 7496, readonly=True, clientId=client_id)
    ...
    for t in tlist:
        ...
        edate = datetime.datetime.today() + datetime.timedelta(days=7)
        expirations = [exp for exp in expirations if exp <= edate.strftime('%Y%m%d')]
        rights = ['P', 'C']
        contracts = [
...
DTE_MAX = 5  # the furthest penny you want to farm
# END USER VARIABLES
